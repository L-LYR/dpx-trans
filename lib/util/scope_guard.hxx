...
class [[nodiscard]] BasicScopeGuard {
 public:
  ...
  constexpr BasicScopeGuard(BasicScopeGuard&& src) : function(src.release()) {}
  constexpr BasicScopeGuard& operator=(BasicScopeGuard&& src) {
    if (this != &src) {
      invoke();
  ...
  constexpr BasicScopeGuard(BasicScopeGuard<G>&& src) : function(src.release()) {}
  ...
  template <typename G>
    requires std::is_convertible_v<G, F>
  constexpr BasicScopeGuard(const G& function_) : function{function_} {}

  template <typename G>
    requires std::is_convertible_v<G, F>
  constexpr BasicScopeGuard(G&& function_) : function{std::move(function_)} {}
