...
void set_thread_name(std::string name) {
  if (name.size() > 16) {
    CRITI("\"{}\" is too long. Name length is restricted to 16 characters.", name);
    return;
  }
...
void bind_core(size_t core_idx) {
  if (core_idx > std::thread::hardware_concurrency()) {
    CRITI("{} is a wrong core index, hardware concurrency is {}", core_idx, std::thread::hardware_concurrency());
    return;
  }
  cpu_set_t cpuset;
  CPU_ZERO(&cpuset);
