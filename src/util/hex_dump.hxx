...
struct Hexdump {
  ...
  Hexdump(const void *data, size_t length)
      : s(reinterpret_cast<const uint8_t *>(data), length) {}

  std::string to_string(size_t row_size = 16, bool show_ascii = true) const;

  friend inline std::ostream &operator<<(std::ostream &out, const Hexdump &d) {
...
template <> struct std::formatter<dpx::Hexdump> : std::formatter<std::string> {
  template <typename Context>
  Context::iterator format(const dpx::Hexdump &dump, Context &ctx) const {
